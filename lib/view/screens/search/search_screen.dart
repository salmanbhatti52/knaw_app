...
import 'package:knaw_news/view/screens/home/post_page.dart';
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: CustomAppBar(
          title: AppData().language!.search,
          leading: Images.arrow_back,
          suffix: Images.search_filter,
          isBack: true,
          isTitle: true,
          suffixTap: () => Get.dialog(SearchFilter(
            tag: tagFilter,
            ...
            isCustom: isDate,
            applyFilter: (tag, date, selected, order, location, dateRange,
                isCustomDate) {
              tagFilter = tag;
              dateFilter = date;
  ...
  Future<void> searchPost() async {
    ...
    response = await DioService.post('news_search_filter', {
      ...
      "orderBy": orderBy,
      if (tagFilter.isEmpty && titleFilter.isEmpty && dateFilter.isEmpty)
        "noFilterPassed": "yes",
      ...
      if (!isDate &&
          dateRangePreset.isNotEmpty &&
          !dateRangePreset.contains("Custom Range"))
  ...
  Future<void> getRecentSearchPost() async {
    ...
    if (response['status'] == 'success') {
      var jsonData = response['data'] as List;
      recentDetail =
          jsonData.map<RecentDetail>((e) => RecentDetail.fromJson(e)).toList();
      totalPost = recentDetail!.length;
      Navigator.pop(context);
      setState(() {});
    } else {
  ...
  Future<void> loadSearchPosts(int postId) async {
    ...
    response = await DioService.post('searched_post_details',
        {"usersId": AppData().userdetail!.usersId, "newsPostId": postId});
    if (response['status'] == 'success') {
      var jsonData = response['data'] as List;
