...
class _MutedMemberState extends State<MutedMember> {
                  ...
                  child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return Column(
                          children: [
                            MuteMemberCard(
                              mutedMemberDetail: mutedMemberDetail![index],
                              onTapSuffix: () {
                                selectedIndex = index;
                                unMuteMember();
                              },
  ...
  void mutedMembersList() async {
    ...
    response = await DioService.post('get_muted_members',
        {"offset": offset, "userId": AppData().userdetail!.usersId});
    if (response['status'] == 'success') {
      totalMember = response['total_count'];
      var jsonData = response['data'] as List;
      mutedMemberDetail!.addAll(jsonData
          .map<MutedMemberDetail>((e) => MutedMemberDetail.fromJson(e))
          .toList());
      isLoading = false;
      setState(() {});
