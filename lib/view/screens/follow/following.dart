...
class _FollowingState extends State<Following> {
                          ...
                          children: [
                            FollowCard(
                              ...
                              isFollower:
                                  widget.userId == AppData().userdetail!.usersId
                                      ? true
                                      : followDetail![index].isFollowingBack ==
                                              "true"
                                          ? false
                                          : true,
                              ...
                              onTap: () => Get.to(() => FollowProfile(
                                    userId:
                                        followDetail![index].followingToUserId,
                                  )),
                            ),
                            Container(
  ...
  Future<void> myFollowingList() async {
    ...
    response = await DioService.post('get_my_following',
        {"offset": offset, "usersId": AppData().userdetail!.usersId});
    if (response['status'] == 'success') {
      totalFollowers = response['total_count'];
      var jsonData = response['data'] as List;
      followDetail!.addAll(
          jsonData.map<FollowDetail>((e) => FollowDetail.fromJson(e)).toList());
  ...
  Future<void> followUser() async {
    ...
    if (response['status'] == 'success') {
      ...
      Navigator.pop(context);
      widget.userId == AppData().userdetail!.usersId
          ? followDetail!.removeAt(followerIndex)
          : followDetail![followerIndex].isFollowingBack == "true"
              ? followDetail![followerIndex].isFollowingBack = "false"
              : followDetail![followerIndex].isFollowingBack = "true";
